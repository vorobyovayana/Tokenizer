...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        self.wfile.write(bytes("""<html><form><body>""", encoding = "cp1251"))
        ...
        for i, file_name in enumerate(cws):
            self.wfile.write(bytes('<li><p>%s</p>' % file_name, encoding = "utf-8"))            
            ...
            for cw in cws[file_name]:
                self.wfile.write(bytes('<li><p>%s</p>' % cw.line, encoding = "cp1251"))
            self.wfile.write(bytes('</ul>', encoding = "cp1251"))
